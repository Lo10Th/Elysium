...
async def list_keys(user: User = Depends(get_current_user)):
    try:
        response = supabase.table("api_keys").select("id, name, created_at, expires_at").eq("user_id", user.id).execute()
...
async def create_key(request: KeyCreate, user: User = Depends(get_current_user)):
    existing = supabase.table("api_keys").select("id").eq("user_id", user.id).eq("name", request.name).execute()
...
async def get_key(key_id: str, user: User = Depends(get_current_user)):
    try:
        response = supabase.table("api_keys").select("id, name, created_at, expires_at").eq("id", key_id).eq("user_id", user.id).single().execute()
...
async def delete_key(key_id: str, user: User = Depends(get_current_user)):
    try:
        response = supabase.table("api_keys").select("id").eq("id", key_id).eq("user_id", user.id).single().execute()
