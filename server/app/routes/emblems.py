...
from app.models import (
    Emblem, EmblemCreate, EmblemUpdate, EmblemVersion, 
    EmblemWithVersion, User
)
...
SCHEMA_PATH = Path(__file__).parent.parent.parent.parent / "schemas" / "emblem.schema.json"
...
async def list_emblems(
    ...
    try:
        ...
        response = query.order("downloads_count", desc=True).range(offset, offset + limit - 1).execute()
        ...
        for row in response.data:
            emblem = Emblem(
                ...
                author_name=row.get("profiles", {}).get("username") if row.get("profiles") else None,
...
async def get_emblem(name: str):
    try:
        response = supabase.table("emblems").select("*, profiles(username)").eq("name", name).single().execute()
...
async def get_emblem_version(name: str, version: str):
    try:
        emblem_response = supabase.table("emblems").select("id").eq("name", name).single().execute()
        ...
        version_response = supabase.table("emblem_versions").select("*").eq("emblem_id", emblem_id).eq("version", version).single().execute()
...
async def create_emblem(
    request: EmblemCreate,
    user: User = Depends(get_current_user)
):
    validate_emblem_yaml(request.yaml_content)
    
    try:
        existing = supabase.table("emblems").select("id").eq("name", request.name).execute()
        
        if existing.data:
            raise HTTPException(status_code=400, detail="Emblem with this name already exists")
        
        emblem_response = supabase.table("emblems").insert({
            "name": request.name,
...
async def update_emblem(
    name: str,
    request: EmblemUpdate,
    user: User = Depends(get_current_user)
):
    validate_emblem_yaml(request.yaml_content)
    
    try:
        emblem_response = supabase.table("emblems").select("*").eq("name", name).single().execute()
        
        if not emblem_response.data:
            raise HTTPException(status_code=404, detail="Emblem not found")
        
        if emblem_response.data["author_id"] != user.id:
            raise HTTPException(status_code=403, detail="Not authorized to update this emblem")
        
        emblem_id = emblem_response.data["id"]
        
        existing_version = supabase.table("emblem_versions").select("id").eq("emblem_id", emblem_id).eq("version", request.version).execute()
...
async def delete_emblem(
    ...
    try:
        ...
        if emblem_response.data["author_id"] != user.id:
            raise HTTPException(status_code=403, detail="Not authorized to delete this emblem")
        
        supabase.table("emblems").delete().eq("id", emblem_response.data["id"]).execute()
